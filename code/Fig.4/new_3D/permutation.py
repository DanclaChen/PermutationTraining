...
from para_init import setup_seed, weights_init_3D, bias_init, coefficient_init_random
...
def permutation_train(j, params, results_queue):
    ...
    actual_width = params.times_w * params.width
...
#     device = torch.device("mps" if torch.backends.mps.is_available() else "cpu")
    ...
    model = NN(layers = [3, actual_width, 1, 1]).to(device)
    print(model)
    ...
    writer = SummaryWriter(comment='3D-bs'+str(params.batch_size)+'-adjust'+str(params.adjust_scale)+'-'+str(params.times_n)+'Xpall-k'+str(params.k_period)+'-merge'+str(params.merge)+'-W'+str(params.width)+'-iter'+str(j))
    ...
    x_train = torch.rand(params.num_train, 3)*2-1

    y_train = torch.unsqueeze(torch.sin(3 * x_train[:, 0]) * torch.cos(1 * x_train[:, 1]) * torch.sin(2 * x_train[:, 2]), 1)
    ...
    temp = torch.linspace(-1, 1, int(np.sqrt(params.num_test)))
    ...
    with torch.no_grad():
        for name, param in model.named_parameters():
            if 'layer1.weight' in name:
                param.copy_(weights_init_3D([actual_width, 3], params.times_w))
    ...
    torch.save(model.state_dict(), './trained_model/3D-'+str(params.times_n)+'Xweight-bs'+str(params.batch_size)+'-k'+str(params.k_period)+'-adjust'+str(params.adjust_scale)+'-merge'+str(params.merge)+'-W'+str(params.width)+'-iter'+str(j)+'.pth')
