...
#     nn.init.uniform_(bias_each, a = -(1+merge), b = 1+merge)
    nn.init.xavier_uniform_(bias_each.unsqueeze(0), gain=nn.init.calculate_gain('relu'))
...
#         print('Before: coe'+str(j)+': '+str(Coe))
    ...
    coe_each = torch.empty(int(shape[1]))
    
    nn.init.xavier_uniform_(coe_each.unsqueeze(0), gain=nn.init.calculate_gain('relu'))

    
#     return coe_each.repeat(times_w)
    return torch.reshape(coe_each, shape)
    
    # shape_half = int(np.divide(shape[1], times_w))
    
    # coe_each = torch.empty(shape_half)
#     nn.init.uniform_(coe_each, a = -(1+merge), b = 1+merge)
#     torch.nn.init.kaiming_normal_(coe_each.unsqueeze(0), a=0, mode='fan_in', nonlinearity='relu')
    # nn.init.xavier_uniform_(coe_each.unsqueeze(0), gain=nn.init.calculate_gain('relu'))
