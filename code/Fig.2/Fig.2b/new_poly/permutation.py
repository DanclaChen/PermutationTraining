...
#     print(model)
    ...
    writer = SummaryWriter(comment='Poly-bs'+str(params.batch_size)+'-adjust'+str(params.adjust_scale)+'-'+str(params.times_n)+'Xpall-k'+str(params.k_period)+'-W'+str(params.width)+'-iter'+str(j))
...
#     y_train = - torch.sin(2 * torch.pi * x_train)
    y_train = 1/2 * (5 * torch.pow(x_train, 3) - 3 * x_train)
...
#     y_test = - torch.sin(2 * torch.pi * x_test)
    y_test = 1/2 * (5 * torch.pow(x_test, 3) - 3 * x_test)
    ...
    torch.save(model.state_dict(), './trained_model/Poly-'+str(params.times_n)+'Xweight-bs'+str(params.batch_size)+'-k'+str(params.k_period)+'-adjust'+str(params.adjust_scale)+'-W'+str(params.width)+'-iter'+str(j)+'.pth')
