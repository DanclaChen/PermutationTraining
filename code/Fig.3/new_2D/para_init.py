...
def weights_init_2D(shape, times_w):
    # initial the weights in the first layer as [1, 1, ..., 1, -1, -1, ..., -1]
    ...
    W_plus = torch.ones(int(shape_half[0]))
    W_minus = -1*W_plus
    W_zero = torch.zeros(int(shape_half[0]))
    
    W = torch.stack([torch.cat([W_plus, W_minus, W_plus, W_minus, W_minus, W_plus,  W_zero, W_zero]), 
                     torch.cat([W_zero, W_zero,  W_plus, W_minus, W_plus,  W_minus, W_plus, W_minus])], dim=1)
