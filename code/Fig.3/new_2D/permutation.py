...
from para_init import setup_seed, weights_init_2D, bias_init, coefficient_init_random
...
def permutation_train(j, params, results_queue):
    ...
    actual_width = 8*params.width
    
    device = "cuda" if torch.cuda.is_available() else "cpu"
#     device = torch.device("mps" if torch.backends.mps.is_available() else "cpu")
    
    
    model = NN(layers = [2, actual_width, 1, 1]).to(device)
    ...
    writer = SummaryWriter(comment='2D-bs'+str(params.batch_size)+'-adjust'+str(params.adjust_scale)+'-'+str(params.times_n)+'Xpall-k'+str(params.k_period)+'-merge'+str(params.merge)+'-W'+str(params.width)+'-iter'+str(j))
    ...
    # x \in [-1, 1]
    x_train = torch.rand(params.num_train, 2)*2-1
    
    y_train = torch.unsqueeze(- torch.sin(torch.pi * x_train[:, 0] * x_train[:, 1]), 1)
    ...
    with torch.no_grad():
        for name, param in model.named_parameters():
            if 'layer1.weight' in name:
                param.copy_(weights_init_2D([actual_width, 2], params.times_w))
    ...
    torch.save(model.state_dict(), './trained_model/2D-'+str(params.times_n)+'Xweight-bs'+str(params.batch_size)+'-k'+str(params.k_period)+'-adjust'+str(params.adjust_scale)+'-merge'+str(params.merge)+'-W'+str(params.width)+'-iter'+str(j)+'.pth')
